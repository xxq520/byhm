<template>
  ...
  <div class="uploadbox" v-else>
    <i v-if="!clear && imgurl && imgurl!=''" class="el-icon-error imgClear" @click="imgClearfn()"></i>
    <i v-if="clear" class="el-icon-error imgClear" @click="closeUpload()"></i>
    <div class="upload" @click="uploadfn($event)">
      <div class="upload-icon" v-if="!imgurl && imgurl==''">
        <i class="el-icon-plus"></i>
        <p>{{ text }}</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    ceartupload(event) {
      ...
      // form.append("base64", event.files[0]);
      // 判断上传图片不能大于10mb
      ...
      reader.onload = function(){
       ...
       let param = {
        ...
        axios({
          method: 'post',
          url: that.Api + 'UnionPayEnter/upLoadPic',
          data:param,
          headers: { 
