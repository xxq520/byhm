<template>
  <div class="glode">
    ...
    <div class="success" v-if="type == 'partner'">
      <div class="succ-img"><img src="@/assets/images/pay_successful.png"></div>
      <div class="succ-text">您的合伙人申请已经提交成功，我们将会安排专人联系您，请注意保持电话畅通，谢谢。</div>
      <div class="button-box">
        <div class="default-btn" @click="goIndex">返回首页</div>
        <div class="default-btn orange" @click="goToPartner">查看申请</div>
      </div>
    </div>
      <div class="success" v-else-if="shaid">
          <div class="succ-img"><img src="@/assets/images/pay_successful.png"></div>
          
          <div class="button-box">
              <div class="default-btn" @click="goshop">返回商家店铺</div>
    ...
    <div class="success" v-else>
     ...
     <!--<div class="succ-text" v-if=" levelid > 1 || auser == 1 ">您可以尊享百业惠盟全国惠盟商家的优惠待遇了，现在，到货真价惠“为爱为家”的网络商城～爱之家商城逛逛吧!因您的身份在爱之家商城已是企业级经销商，所以不能再赠送抵扣券，可您的身份可以享受更优惠的价格。</div>
      ...
      <div class="button-box" >
        ...
        <div class="default-btn orange" @click="goToOrder1">查看订单</div>
      </div>
...
export default {
  data() {
      return {
        ...
        shaid:sessionStorage.getItem('shaid'),
  ...
  methods: {
   //返回商家详情
   goshop(){
       this.$router.push({path:'/shopDetails',query:{id:this.shaid,code:this.code}})
   },
    //成功后返回我的页面
    goMineIndex() {
      let that = this
      ...
      //},5000);
       ...
       goIndex() {
        if(this.backshqPd||this.$route.query.platform == 'android'||this.$route.query.platform == 'ios'){  
    ...
    goToPartner(){
      ...
      if(this.partner){
        this.$router.push({name: 'partnerInfor',query:{status:0}})
      }else{
        Toast('你还没有申请为合伙人!');
      };
    }
