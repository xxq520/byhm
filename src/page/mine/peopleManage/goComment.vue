<template>
    <div class="push-comment">
        ...
        <div class="score">
            ...
            <div>
                <el-rate v-model="value1" void-color="#c3c3c3" :colors="['#f86b1f','#f86b1f','#f86b1f']"></el-rate>
            </div>
        </div>
        ...
        <div class="comment-pic">
            ...
            <imgUpload :add="true" :commentPhone="true" text="添加图片" :stop.sync="imgUploadStopAdd"
                @img-upload="shopImgChange" @click.native="shopImgAdd"></imgUpload>
        </div>
        <!-- <div class="comment-label">
            <div class="title">添加标签</div>
            <div class="label-box">
                <div class="label" v-for="(item,index) in label" :key="index" @click="switchLabel(item)"
                    :class="{'active':activefn(item)}">{{item}}</div>
            </div>
        </div> -->
        <div class="anonymous">
        ...
        methods: {
                ...
                axios({
                    ...
                    if (res.data.code == 200) {
                        ...
                        for (let i = 0; i < data.orderDetails.length; i++) {
                            this.goodsIds.push(data.orderDetails[i].id)   
                        }
                        this.goodsIds = this.goodsIds.join(',')
            ...
            publishfn() {
                ...
                let post = {
                    ...
                    username: that.$cookie.get('realName'),
                    rate: that.value1,
                    content: that.content,
                    anonymous: that.anonymous,
                    seq: this.seq,
                    strings: that.shopImg,
                    orderSn:this.orderSn,
                ...
                Indicator.open();
                axios({
                    url: this.Api1 + '/shop/merchantComment/saveComment',
                    data: post,
                    method: "post",
                    headers: {
                        "Content-Type": "application/json; charset=UTF-8",
