    ...
    <div class="InvitMember">
        <header class="mint-header" id="headerInvait">
            ...
            <h1 class="mint-header-title">
                邀请成为{{this.$route.query.exper == 'province'?'省':this.$route.query.exper == 'city'?'市':'县'}}合伙人</h1>
        </header>
        <!-- 底部海报轮播 -->
        <div class="swiper-container">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="proxy imgbox">
                        <img :src="imgUrl1" class="proxy_bg" >
                    </div>
                </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                mySwiperIndex: 0,
                imgUrl1:'',
                exper:this.$route.query.exper,
                level: this.$cookie.get("level_id")
            }
        },
        mounted() {
            if (this.$route.query.exper == 'province') {
                $('.InvitMember .swiper-slide .name').css({});
            }
            let that = this;
            let exper = this.$route.query.exper;
            this.$nextTick(function () {
                ...
                // if (exper) {
                //     this.uri = host + '/#/partnerApplication/' + this.inviteCode + '/' + this.$cookie
                //         .get('level_id') + '/' + this.$route.query.exper + '/1';
                // };
                ...
                swiperbox.swiperTouch(function () {
                    ...
                    $('.swiperIndex span').removeClass('hover');
                    $('.swiperIndex span').eq(this.index).addClass('hover');
                });
                ...
                // // 微信分享参数配置
                axios.post('http://by.520shq.com/api/wechat/wechatParam?url=' + encodeURIComponent(
                    window.location.href.split('#')[0])).then(res => {
                    if (res.status == 200) {
                        let data = res.data
        ...
        methods: {
                       getimage(){
        let inviteType = this.exper=="province"?5:this.exper=="city"?6:7
                       let params = {
                       ...
                       if (res.status == 200) {
                       this. imgUrl1=res.data.data.inivteImageUrl;
            ...
            copyUrl() {
                ...
                if (sessionStorage.getItem('applogin') || this.$route.query.platform == 'android' || this.$route.query
                    .platform == 'ios') {
                    let desc = '百业惠盟欢迎您——';
                    window.action.ShareLinks(this.shareTitle, desc, this.imgUrl, this.uri);
                } else {
...
<style scoped>
    ...
    .InvitMember .swiper-slide:nth-child(2) .qrcode,
    .InvitMember .swiper-slide:nth-child(3) .qrcode {
        top: 55%;
    }

    /* 微信名称及二维码 */
    .InvitMember .swiper-slide .name {
        font-size: 0.558rem;
        color: #fff;
        position: absolute;
        top: 81%;
    ...
    .InvitMember .swiper-slide:nth-child(2) .name,
    .InvitMember .swiper-slide:nth-child(3) .name {
        top: 49%;
    }

    /* .InvitMember .swiper-slide:nth-child(1) .name{
    top: 47.5%;
  } */
    /* 底部按钮区域 */
    .InvitMember .ivtBtn {
