<template>
   <div class="InvitShop">
     <header class="mint-header" id="headerInvait">
        ...
        <h1 class="mint-header-title">邀请优质商家加盟平台</h1>
    </header>
    <!-- 底部海报轮播 -->
    <div class="swiper-container">
        <div class="swiper-wrapper">
            <div class="swiper-slide">
                <div class="proxy" id="capture">
...
export default {
   ...
   mounted(){
     let that = this;
      this.$nextTick (function () {
        ...
        // this.uri = host+'/#/application/'+this.inviteCode
        //swiper轮播
        ...
        $('.proxy').height($(window).height()-$('.ivtBtn').height()-$('#headerInvait').height()-20);
        let img = new Image();
        ...
        // // 微信分享参数配置
        axios.post('http://by.520shq.com/api/wechat/wechatParam?url='+window.location.href.split('#')[0]).then(res=>{
          if (res.status == 200) {
            let data = res.data
            ...
            wx.config({
              ...
              jsApiList: ['onMenuShareAppMessage', 'onMenuShareTimeline','translateVoice','onMenuShareQQ','onMenuShareQZone'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
            })
          }else{
            Toast('网络错误，请重试！')
          }
   ...
   methods: {
    ...
    getCountyNum() {
      let that = this
      if(that.$cookie.get('level_id') == 3) {
        axios({
          method: 'get',
          url: that.Api + 'merchant/queryCount?userId=' + that.$cookie.get('userid')
        }).then(res => {
          console.log(res)
          if(res.data >= 150) {
            MessageBox.alert('谢谢您的努力！本区/县惠盟商家开发奖励指标已用完，您成功介绍商家已不会再有开发奖金，敬请您知悉。谢谢您！，日后此区县级再有名额出来都不给开发商户奖励了。').then(action => {
...
color:#fff;
  ...
  .InvitShop .pop1 .shareItem .title{
    /* font-size: .6667rem;
    color: #EC0746;
    text-align: center; */
    width: 100%;
  ...
  .InvitShop .pop1 .shareItem .group ul{
    overflow: hidden;
    width: 15.3333rem;
    margin: 1.3333rem auto 1rem;
  }
  .InvitShop .pop1 .shareItem .group ul li{
    float: left;
    width: 20%;
    display: flex;
    text-align: center;
  }
  .InvitShop .pop1 .shareItem .group ul li img{
    width: 2rem;
    height: 2rem;
  }
  .InvitShop .pop1 .shareItem .group ul li .name{
    font-size: .6222rem;
    color: #666666;
  }
  .InvitShop .pop1 .shareItem .group ul li a{
    display: block;
  }
  .InvitShop .pop1 .cancelBtn{
    width: 100%;
    height: 2rem;
  ...
  .InvitShop .swiper-slide .proxy {
      background-repeat: no-repeat;
      background-size: contain;
      transform: scale(0.9);
      opacity: .7;
  }
  .InvitShop .iActive .proxy {
      transform: scale(1);
      opacity: 1;
      z-index: 10;
  }
  .InvitShop .swiper-slide .proxy img.proxy_bg{
      width:auto;
      height:100%;
  }
  .InvitShop .swiper-slide .ewm {
  ...
  .InvitShop .swiper-slide .qrcode >>> img{
    width: auto;
    height:100%;
    margin:0 auto;
  }
  ...
  /* 微信名称及二维码 */
  .InvitShop .swiper-container .swiper-wrapper .swiper-slide .name{
    font-size: .55rem;
    color: #fff;
    position: absolute;
    top: 84%;
  ...
  .InvitShop .swiper-container .swiper-wrapper .swiper-slide .n2{
    top: 93%;
  }
  .InvitShop .swiper-container .swiper-wrapper .swiper-slide .n3{
    top: 35%;
  }
  ...
  .InvitShop .ivtBtn img{
      width: .7111rem;
      height: .7111rem;
      margin-right: .4rem;
      margin-bottom: -1px;
  }
  .InvitShop .ivtBtn .btn{
  ...
  .InvitShop .ivtBtn .btn:first-child{
      background-color: #ff9408;
      outline: none;
  }
  .InvitShop .ivtBtn .btn:nth-child(2){
      float: right
  }
